...
export function deriveLbPair(
  ...
  // TODO: Fetch LB_PAIR_SEED from contract constants
  return PublicKey.findProgramAddressSync(
    [
      Buffer.from("lb_pair"),
      tokenX.toBuffer(),
...
export function derivePosition(
  ...
  if (width <= 0) {
    throw new Error("Position width must be positive");
  }
  const lowerBinIdBuffer = Buffer.alloc(4);
  lowerBinIdBuffer.writeInt32LE(lowerBinId);

  const widthBuffer = Buffer.alloc(4);
  widthBuffer.writeInt32LE(width);
